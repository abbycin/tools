...
public:
	...
	void consume(const char *buffer, size_t len)
	{
		...
		for (i = 0; i < len; i++) {
			...
			case START_BOUNDARY:
				...
				if (c != boundary_[index + 2]) {
					set_error("Malformed. Found different "
						  "boundary data than the "
						  "given one.");
					return;
				}
				index++;
			...
			case HEADER_FIELD:
				...
				if (c == COLON) {
					if (index == 1) {
						// empty header field
						set_error("Malformed first "
							  "header name "
							  "character.");
						return;
					}
					data_callback(on_header_field_,
						      header_field_mark_,
						      buffer,
						      i,
						      len,
			...
			case HEADER_VALUE:
				if (c == CR) {
					data_callback(on_header_value_,
						      header_value_mark_,
						      buffer,
						      i,
						      len,
			...
			case HEADER_VALUE_ALMOST_DONE:
				if (c != LF) {
					set_error("Malformed header value: LF "
						  "expected after CR");
					return;
				}
			...
			case HEADERS_ALMOST_DONE:
				if (c != LF) {
					set_error("Malformed header ending: LF "
						  "expected after CR");
					return;
				}
			...
			case PART_DATA:
				...
				if (index == 0) {
					i += boundaryEnd;
					// boyer-moore derived algorithm to
					// safely skip non-boundary data
					while (i < len &&
					       !is_in_boundary(buffer[i])) {
						i += boundary_size_;
					}
					i -= boundaryEnd;
					c = buffer[i];
				...
				if (index > 0) {
					// when matching a possible boundary,
					// keep a lookbehind reference in case
					// it turns out to be a false lead since
					// index is unsigned, this can happen
					// either index equal 0 or index far
					// more larger than lookbehind_size_
					if (index - 1 >= lookbehind_size_) {
						set_error("Parser bug: index "
							  "overflows "
							  "lookbehind buffer. "
							  "Please send bug "
							  "report with input "
					...
					// if our boundary turned out to be
					// rubbish, the captured lookbehind
					// belongs to partData
					...
					// reconsider the current character even
					// so it interrupted the sequence it
		...
		data_callback(on_header_value_,
			      ...
			      false);
		data_callback(
			on_part_data_, part_data_mark_, buffer, i, len, false);
...
private:
	...
	std::string gen_temp_name(const std::string &prefix)
	{
		...
		res.append(prefix);
		static const char seed[] = "abcdefghijklmnopqrstuvwxyz123456789"
	...
	void reset_callbacks()
	{
		...
		on_header_field_ =
			[this](const char *buf, size_t beg, size_t end)
	...
	void data_callback(CallBack &cb,
			   size_t &mark,
			   const char *buf,
			   size_t i,
			   size_t buf_len,
