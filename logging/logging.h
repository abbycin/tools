...
namespace meta
{
	...
	public:
		...
		void push(std::string &&data)
		{
			...
			auto old_head =
				head_.exchange(tmp, std::memory_order_acq_rel);
	...
	public:
		...
		void write(std::string &buffer)
		{
			...
			fwrite_unlocked(
				time_buf_, 1, sizeof(time_buf_) - 1, fp_);
			while (rest != 0) {
				write_bytes =
					fwrite_unlocked(data, 1, rest, fp_);
				data += write_bytes;
				rest -= write_bytes;
			}
	...
	private:
		...
		void link()
		{
			...
			int res = symlink(new_name().c_str(),
					  current_log_.c_str());
			(void)res;
		}
	...
	private:
		...
		{
			fmt_len_ = snprintf(
				fmt_buf_, sizeof(fmt_buf_), format, data);
	...
	private:
				   ...
				   bool is_sync)
		{
			...
			if (is_sync)
				...
				log_async(
					fp, path, prefix, interval, is_stdout);
		}

		void log_sync(FILE *fp,
			      const char *path,
			      const char *prefix,
			      long interval,
			      bool is_stdout)
...
public:
	...
	static bool create_async(FILE *fp = stderr,
				 const char *path = "",
				 const char *prefix = "",
	...
	static bool create_sync(FILE *fp = stderr,
				const char *path = "",
				const char *prefix = "",
				long interval = 60 * 60 * 24)
	{
	...
	{
		ss_ << ' ' << get_tid() << MSG[level] << basename(file) << ':'
		    << line << ' ';
		if (func)
			ss_ << '`' << func << "` ";
	}
