...
namespace nm
{
  namespace meta
  {
    ...
    public:
      ...
      FileLog(const std::string& path, const std::string& prefix, long interval)
          : path_(path), interval_(interval), count_(now()), time_cache_(count_), last_roll_(count_), fp_(nullptr)
      {
        if(path_.empty())
        {
          path_ = "./";
      ...
      FileLog(FILE* fp) : path_(), interval_(-1), count_(now()), time_cache_(count_), last_roll_(count_), fp_(fp) {}
      ...
      void flush() { fflush_unlocked(fp_); }
    ...
    private:
      ...
      void update_time()
      {
        ...
        snprintf(time_buf_, sizeof(time_buf_), "%04d%02d%02d %02d:%02d:%02d.%06ld", tm_.tm_year + 1900, tm_.tm_mon + 1,
                 tm_.tm_mday, tm_.tm_hour, tm_.tm_min, tm_.tm_sec, tv_.tv_usec);
      }

      void update_micro() { snprintf(time_buf_ + 18, 7, "%06ld", tv_.tv_usec); }
      ...
      std::string& new_name()
      {
        ...
        int res = snprintf(buf, sizeof(buf), "%04d%02d%02d-%02d%02d%02d.%d.log", tm_.tm_year + 1900, tm_.tm_mon + 1,
                           tm_.tm_mday, tm_.tm_hour, tm_.tm_min, tm_.tm_sec, getpid());
        name_cache_ = path_;
    ...
    public:
      ...
      Stream& operator<<(const std::nullptr_t&) { return *this; }
      ...
      Stream& operator<<(const unsigned char* s) { return this->operator<<(reinterpret_cast<const char*>(s)); }
      ...
      Stream& operator<<(const float data) { return this->fmt("%.6g", data); }

      Stream& operator<<(const double data) { return this->fmt("%.10g", data); }
      ...
      std::string str() { return {buffer_, pos_}; }
    ...
    {
    public:
      LoggerBackend() : queue_(), log_(nullptr), tid_() {}

      ~LoggerBackend() { this->join(); }
      ...
      void init(FILE* fp, const char* path, const char* prefix, long interval, bool is_sync)
      {
      ...
      void consume(StreamBuffer& ss) { writer_(ss); }
    ...
    private:
      ...
      void create_logger(FILE* fp, const char* path, const char* prefix, long interval, bool is_sync)
      {
        if(path == nullptr)
          path = "";
        if(prefix == nullptr)
      ...
      void log_sync(FILE* fp, const char* path, const char* prefix, long interval, bool is_stdout)
      {
        if(is_stdout)
        {
      ...
      void thread_func()
      {
        ...
        while(running_)
        {
          ...
          cond_.wait_for(lk, timeout_, [this, &data] { return queue_.try_pop(data) || !running_; });
          need_notify_ = false;
          log_->write(data);
          log_->flush();
        }
      ...
      void log_async(FILE* fp, const char* path, const char* prefix, long interval, bool is_stdout)
      {
        if(is_stdout)
        {
          log_.reset(new FileLog(fp));
          log_->init();
          tid_ = std::thread([this] { thread_func(); });
        }
        else
        {
          log_.reset(new FileLog(path, prefix, interval));
...
#define disable_log_ true
  ...
  public:
    ...
    static bool create_async(FILE* fp = stderr, const char* path = "", const char* prefix = "",
    ...
    static bool create_sync(FILE* fp = stderr, const char* path = "", const char* prefix = "",
                            long interval = 60 * 60 * 24)
    {
      if(disable_log_)
        return true;
    ...
    {
      ss_ << ' ' << get_tid() << MSG[level] << basename(file) << ':' << line << ' ';
      if(func)
        ss_ << '`' << func << "` ";
    }
    ...
    template<size_t LEN>
    Logger& operator<<(const char (&a)[LEN])
    {
      ss_.append(a, LEN - 1);
      return *this;
    }
  ...
  private:
    ...
    static inline const char* MSG[] = {" [INFO]  ", " [WARN]  ", " [DEBUG] ", " [ERROR] ", " [FATAL] "};
