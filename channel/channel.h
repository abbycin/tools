...
void dealloc(void* x) { scalable_free(x); }
...
template<typename T>
using Queue_t = tbb::concurrent_queue<T>;
#else
template<typename T>
...
public:
  ...
  void try_lock() { flag_.test_and_set(std::memory_order_acquire); }
...
public:
  Queue() : head_(alloc<Node>()), tail_(head_.load())
  {
    auto tmp = head_.load();
...
public:
  ...
  bool try_recv(T& data) { return queue_.try_pop(data); }
  ...
  {
    LockGuard<Mutex> lk(mtx_);
    cond_.wait_for(lk, timeout, [this] { return !queue_.empty(); });
...
public:
  ...
  void send(const T& data) { sender_->send(data); }

  void send(T&& data) { sender_->send(std::move(data)); }

  Sender clone() { return *this; }

private:
  Sender(std::shared_ptr<ReceiverImpl<T>> recv) : sender_(recv) {}
...
public:
  ...
  void recv(T& data) { recv_->recv(data); }

  bool try_recv(T& data) { return recv_->try_recv(data); }
...
private:
  Receiver(ReceiverImpl<T>* recv) : recv_(recv) {}

  Receiver(const Receiver&) = delete;

  Receiver& operator=(const Receiver&) = delete;

  std::shared_ptr<ReceiverImpl<T>> get() { return recv_; }
