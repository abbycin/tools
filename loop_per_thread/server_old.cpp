...
public:
  Worker()
      : rbuf_(0), wbuf_(0), reader_(io_), writer_(io_), read_buf_(&rbuf_, sizeof rbuf_),
        write_buf_(&wbuf_, sizeof wbuf_), thread_([this] { thread_func(); })
  {
  }
  ...
  void enqueue(SessionPtr session) { enqueue_writer(session); }
...
private:
  ...
  void enqueue_reader()
  {
    reader_.async_read_some(read_buf_, [this](const error_code& ec, size_t bytes) { read_handle(ec, bytes); });
  }
