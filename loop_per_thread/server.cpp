...
public:
  ...
  public:
    ...
    bool operator==(const iterator& rhs) { return data_ == rhs.data_; }
  ...
  Circle(size_t capacity) : size_(capacity), circle_(nullptr)
  {
  ...
  iterator iter() { return iterator(circle_); }
...
{
public:
  static std::shared_ptr<Session> make_session(io_service& io) { return std::make_shared<Session>(io); }
...
private:
  ...
  void processing()
  {
    async_write(socket_, buf_, [this, self = shared_from_this()](const error_code& ec, size_t) {
      if(ec)
      {
        // std::cerr << ec.message() << std::endl;
        return;
...
{
public:
  Worker() : circle_(std::thread::hardware_concurrency()), iter_(circle_.iter())
  {
  ...
  io_service& get_io_service() { return iter_->get_io_service(); }

  void enqueue(SessionPtr session) { dispatch(session); }
...
private:
  ...
  {
  public:
    Task() : strand_(io_), work_(new io_service::work(io_)) {}
    ...
    {
      strand_.dispatch([session] { session->run(); });
    }
...
private:
  ...
  void accept_handle(SessionPtr session, WorkerPtr worker, const error_code& ec)
  {
    if(!ec)
    {
      ++cur_;
  ...
  void start_accept()
  {
    ...
    acceptor_.async_accept(session->socket(),
                           [this, session, worker](const error_code& ec) { accept_handle(session, worker, ec); });
  }
};
...
{
public:
  Server(tcp::endpoint&& ep, unsigned long num = 1) : sg_(io_), manager_(io_, std::move(ep), num)
  {
    sg_.add(SIGINT);
    sg_.add(SIGQUIT);
    sg_.add(SIGTERM);
