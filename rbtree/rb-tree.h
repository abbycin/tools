	...
	void insert_fixup(rbnode *t, node *x)
	{
		...
		while (x->parent->color == RED) {
			if (x->parent == x->parent->parent->lhs) {
				...
				if (y->color ==
				    RED) // a red node can't have red child
				{
					/*
					 *  BEFORE INSERT, THE TREE IS SATISFY
					 * RED-BLACK TREE'S PROPERTIES THUS, ppa
					 * IS BLACK, AND WE DON'T CARE x IS LEFT
					 * CHILD OR RIGHT CHILD. ppa(black)
					 * ppa(red)
					 *        /         \               / \
