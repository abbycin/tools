...
public:
	...
	constexpr static size_t uchar_max =
	...
	// member functions
	constexpr basic_string_view() : data_ { nullptr }, len_ { 0 }
	{
	}

	template<typename T>
	explicit basic_string_view(const T &t)
		: data_ { t.data() }, len_ { t.size() }
	{
	}
	...
	{
		int r = basic_string_view<StringType>::cmp(
			data_, x.data_, std::min(len_, x.len_));
		if (r == 0) {
			if (len_ < x.len_) {
				r = -1;
			} else if (len_ > x.len_) {
	...
	basic_string_view &remove_prefix(size_type n)
	{
		data_ += n;
		len_ -= n;
		return *this;
	...
	basic_string_view &remove_suffix(size_type n)
	{
		len_ -= n;
		return *this;
	}
	...
	{
		basic_string_view res = *this;
		if (this->empty()) {
			return {};
		}

		while (*res.data() == ' ' || *res.data() == '\r' ||
		       *res.data() == '\n' || *res.data() == '\t') {
			res.remove_prefix(1);
		}
		while (!res.empty()) {
	...
	{
		return ((len_ >= x.len_) &&
			(cmp(data_ + (len_ - x.len_), x.data_, x.len_) == 0));
	}

	basic_string_view substr(size_type pos, size_type size = npos) const
	{
		if (pos > this->size()) {
			pos = this->size();
		}
	...
	size_type find(const basic_string_view &x, size_type pos = 0) const
	{
		...
		auto r = std::search(
			this->begin() + pos, this->end(), x.begin(), x.end());
		const size_type rpos =
			static_cast<size_type>(r - this->begin());
	...
	size_type rfind(const basic_string_view &x, size_type pos = npos) const
	{
		...
		auto last = this->begin() +
			std::min(this->size() - x.size(), pos) + x.size();
		auto r = std::find_end(this->begin(), last, x.begin(), x.end());
		return r != last ? static_cast<size_type>(r - this->begin())
				 : npos;
	}
	...
	size_type rfind(char x, size_type pos = npos) const
	{
		...
		for (size_type i = std::min(pos, this->size() - 1); i > 0;
		     --i) {
			if (this->data()[i] == x) {
				return i;
			}
	...
	size_type find_first_of(const basic_string_view &x,
				size_type pos = 0) const
	{
		...
		for (size_type i = pos; i < this->size(); ++i) {
			if (lookup[static_cast<unsigned char>(
				    this->data()[i])]) {
				return i;
			}
		}
	...
	size_type find_first_not_of(const basic_string_view &x,
				    size_type pos = 0) const
	{
		if (this->size() == 0) {
			return npos;
	...
	size_type find_last_of(const basic_string_view &x,
	...
	size_type find_last_not_of(const basic_string_view &x,
				   size_type pos = npos) const
	{
		if (this->size() == 0) {
			return npos;
...
{
	auto r = string_view::cmp(
		x.data(), y.data(), std::min(x.size(), y.size()));
...
constexpr string_view operator""_sv(const string_view::value_type *data,
				    string_view::size_type len)
{
	return { data, len };
}

inline int compare_i(const string_view &l, const string_view &r)
{
	auto lb = l.begin();
	auto rb = r.begin();
	while (lb != l.end() && rb != r.end()) {
		if (*lb != *rb && std::tolower(*lb) != std::tolower(*rb)) {
			break;
		}
		++lb;
		++rb;
