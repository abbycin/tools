...
#define put(x) do { std::cerr << "put: " << x << '\n'; r.put(x); } while(0)
#define has_prefix(x) do { std::cerr << "has_prefix: " << x  << "? " << (r.has_prefix(x) ? "true" : "false") << '\n'; } while(0)
#define has_word(x) do { std::cerr << "has_word: " << x << "? " << (r.has_word(x) ? "true" : "false") << '\n'; } while(0)
