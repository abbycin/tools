...
namespace nm
{
  namespace meta
  {
    ...
    struct test_arg<T>
    {
      constexpr static bool value =
          !std::is_reference_v<T> && std::is_copy_assignable_v<T> && std::is_constructible_v<T>;
    };
    ...
    struct Inspector<R(Args...)>
    {
      constexpr static bool is_r_valid =
          std::is_void_v<R> || (std::is_copy_assignable_v<R> && std::is_constructible_v<R>);
    ...
    struct call_impl
    {
      ...
      {
        *res = std::forward<F>(f)(
            std::forward<std::tuple_element_t<Is, std::remove_reference_t<TupleType>>>(std::get<Is>(args))...);
      }
    };
    template<>
  ...
  public:
    ...
    ~FunctorMap() { this->clear(); }

    FunctorMap(const FunctorMap&) = delete;
    FunctorMap& operator=(const FunctorMap&) = delete;

    FunctorMap(FunctorMap&& rhs) noexcept : functors_{std::move(rhs.functors_)}, tidy_{std::move(rhs.tidy_)} {}
    ...
    bool contain(const std::string& name) { return functors_.count(name) > 0; }
