...
namespace nm
{
  namespace meta
  {
    template<typename...> struct test_arg;
    template<> struct test_arg<>
    {
      constexpr static bool value = true;
    };
    template<typename T> struct test_arg<T>
    {
      constexpr static bool value = !std::is_reference_v<T> && std::is_copy_assignable_v<T> && std::is_constructible_v<T>;
    };
    template<typename T, typename... Ts> struct test_arg<T, Ts...>
    {
      constexpr static bool value = test_arg<T>::value && test_arg<Ts...>::value;
    };
    ...
    {
      constexpr static bool is_r_valid = std::is_void_v<R> || (std::is_copy_assignable_v<R> && std::is_constructible_v<R>);
      constexpr static bool is_arg_valid = test_arg<Args...>::value;
      static_assert(is_r_valid, "invalid return type, must be `void` or can be copied and constructed");
      static_assert(is_arg_valid, "invalid argument type, must be copyable and can't be reference");
  ...
  class FunctorMap
  {
    public:
      ...
      FunctorMap(FunctorMap&& rhs) noexcept
        : functors_{std::move(rhs.functors_)}, tidy_{std::move(rhs.tidy_)}
      {}
