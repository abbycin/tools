...
public:
	...
	string_ext &
	replace(const string_ext &oldstr, const string_ext &newstr, long count)
	{
	...
	void split(Container &res,
		   const string_ext &sep = string_ext(),
		   long max = -1) const
	{
		if (max < 0)
		...
		if (sep_len == 0) {
			...
			while (max > 0 && iter != end()) {
				if (this->substr(iter - begin(), sep_len) ==
				    sep) {
					res.emplace_back(this->substr(
						cur - begin(), iter - cur));
					iter += sep_len;
...
private:
	...
	public:
		...
		Parser(Base *x)
			: arg_(x)
			, parser_([](const string_ext &str, void *res)
