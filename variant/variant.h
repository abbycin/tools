...
namespace meta
{
	...
	struct max_size_of<_1, _2, Rest...> {
		constexpr static size_t value = _1 > _2
			? max_size_of<_1, Rest...>::value
	...
	struct helper_and<_1, _2, R...> {
		constexpr static bool value =
			_1::value ? helper_and<_2, R...>::value : _1::value;
	};
	...
	struct helper_or<_1, _2, R...> {
		constexpr static bool value =
			_1::value ? _1::value : helper_or<_2, R...>::value;
	};
	...
	struct is_valid_variant_types<T> {
		constexpr static bool value =
			helper_and<helper_not<std::is_void<T>>,
	...
	struct is_valid_variant_types<T, U> {
		constexpr static bool value =
			helper_and<is_valid_variant_types<T>,
	...
	struct is_type_unique {
		...
		constexpr static bool value =
			tmp ? is_type_unique<Rest...>::value : tmp;
	};
	...
	struct ctor_type_helper<T, U> {
		using type = typename cond<std::is_constructible<U, T>::value,
					   U,
					   T>::type;
	};
	template<typename T, typename U, typename... Rest>
	struct ctor_type_helper {
		using type = typename ctor_type_helper<
			typename ctor_type_helper<T, U>::type,
			Rest...>::type;
	};
	...
	public:
		constexpr static bool value =
			helper_and<helper_not<std::is_const<Raw>>,
				   decltype(test(std::forward<Rhs>(
					   std::declval<Rhs>())))>::value;
	};
	...
	struct acceptable_type_helper<T, U> {
		using type =
			typename cond<is_acceptable<U, T>::value, U, T>::type;
	};
	template<typename T, typename U, typename... Rest>
	struct acceptable_type_helper {
		using type = typename acceptable_type_helper<
			typename acceptable_type_helper<T, U>::type,
			Rest...>::type;
	};
	...
	public:
		using type = typename cond<
			is_in<Raw, U, Rest...>::value,
			Raw,
			typename acceptable_type_helper<T, U, Rest...>::type>::
			type;
	};
	...
	struct is_acceptable_from<T, U> {
		constexpr static bool value =
			std::is_same<typename acceptable_type<T, U>::type,
				     U>::value;
	};
	template<typename T, typename U, typename... Rest>
	struct is_acceptable_from {
	...
	public:
		constexpr static int value =
			ok ? 0 : 1 + index_of_type_helper<T, Rest...>::value;
	};

	template<typename T, typename... Rest>
	struct index_of_type {
	private:
		constexpr static int tmp =
			index_of_type_helper<T, Rest...>::value;
	...
	public:
		...
		using iarg =
			typename std::remove_cv<typename std::remove_reference<
				typename extract<F>::iarg>::type>::type;
	...
	struct variant_call {
		...
		static void invoke(Type &arg, F &&f)
		{
			variant_call_wrapper<is_type_match<decltype(f), Type>::
						     arg_same>::invoke(arg, f);
		}
	...
	struct variant_helper<T, R...> {
		...
		static void
		copy(int index, void *lhs, const void *rhs, size_t n)
		{
			if (index > 0) {
				index -= 1;
		...
		static void call(int index, void *data, Fn &&...f)
		{
			if (index > 0) {
				index -= 1;
				variant_helper<R...>::call(
					index, data, std::forward<Fn>(f)...);
			} else if (index == 0) {
				variant_call::invoke<T, Fn...>(
					*static_cast<T *>(data),
					std::forward<Fn>(f)...);
			}
		}
		...
		static Res apply(int index, void *data, Obj &obj)
		{
			if (index > 0) {
				index -= 1;
				return variant_helper<
					R...>::template apply<Res, Obj>(index,
									data,
									obj);
			} else if (index == 0) {
...
public:
	...
	template<typename T,
		 typename std::enable_if<
			 and_<not_<std::is_same<T, variant>>,
			      meta::is_construct_from<const T &, Rest...>>::
				 value>::type * = nullptr>
	...
	template<typename T,
		 typename std::enable_if<and_<
			 std::is_rvalue_reference<T &&>,
			 not_<std::is_const<T>>,
			 not_<std::is_same<T, variant>>,
			 meta::is_construct_from<T &&, Rest...>>::value>::type
			 * = nullptr>
	...
	variant(variant &&rhs) noexcept : variant()
	{
		move_construct(rhs);
	}
	...
	{
		using Type = typename meta::acceptable_type<const T &,
							    Rest...>::type;
	...
	template<typename T,
		 typename std::enable_if<
			 meta::is_acceptable_from<T &&, Rest...>::value>::type
			 * = nullptr>
	variant &operator=(T &&rhs)
	{
		using Real = decltype(rhs);
		using Type =
			typename meta::acceptable_type<Real, Rest...>::type;
		constexpr int tmp = meta::index_of_type<Type, Rest...>::value;
		if (type_index_ != tmp) {
			clear();
	...
	template<typename T,
		 typename std::enable_if<meta::is_in<T, Rest...>::value>::type
			 * = nullptr>
	...
	template<typename T>
	T &get(typename std::enable_if<meta::is_in<T, Rest...>::value>::type * =
		       nullptr)
	{
		if (type_index_ < 0)
	...
	{
		return helper::template apply<typename Obj::type, Obj>(
			type_index_, data_.raw(), obj);
	}

	template<typename Res, typename Obj>
	Res apply(Obj &&obj)
	{
		return helper::template apply<Res, Obj>(
			type_index_, data_.raw(), obj);
	}
...
private:
	...
	constexpr static size_t data_size =
		meta::max_size_of<sizeof(Rest)...>::value;
